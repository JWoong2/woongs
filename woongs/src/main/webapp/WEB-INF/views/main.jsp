...
function afterAjax() {
    ...
    $.ajax({
		...
		,dataType:"json" })
		.done(function(args){	//응답이 성공 상태 코드를 반환하면 호출되는 함수
			console.log("결과값이 없나 본데");
		 	for(var i=0; i < args.length; i++) {
			 	console.log(args[i].category_first);
			 	$("#"+args[i].category_first).after("<div id='"+args[i].no+"'><a href='/chatAsk?user_id="+args[i].user_id+"&&no="+args[i].no+"'>문의하기</a><img src="+args[i].thumbnail+">"+args[i].title+"</div>");
		 	}
				})
    	.fail(function(e) {
	    	alert(e.responseText);
